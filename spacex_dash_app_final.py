...
#Get a list of all launch sites in the required format for a dropdown menu
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(spacex_df[spacex_df['class'] == 1].groupby("Launch Site", as_index = False).count(), 
                     ...
                     names='Launch Site', 
                     title='Succesful launches across all sites')
        return fig
    else:
...
def get_bar_chart(entered_site, entered_payload):
    if entered_site == 'ALL':
